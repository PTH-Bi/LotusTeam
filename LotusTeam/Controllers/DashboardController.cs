...
namespace LotusTeam.API.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        #region Work Report Dashboard

        /// <summary>
        /// Lấy báo cáo công việc (thống kê hiệu suất làm việc)
        /// </summary>
        [HttpGet("work-report")]
        [Authorize]
        public async Task<ActionResult<ApiResponse<WorkReportDto>>> GetWorkReport()
        {
            try
            {
                var userRole = User.FindFirst(ClaimTypes.Role)?.Value ?? "";
                ...
                var workReport = new WorkReportDto
                {
                    OntimeCount = 742,
                    LateCount = 110,
                    OntimeTrend = 12,
                    LateTrend = -6,
                    AvgWorkingHours = 4.2m,
                    ...
                    TeamVelocity = 8.4m,
                    TeamVelocityTrend = 1.2m,
                    TopPerformers = await GetTopPerformers(userRole, currentUserId)
                };

                return Ok(new ApiResponse<WorkReportDto>
                {
                    Success = true,
                    Message = "Báo cáo công việc",
                    Data = workReport
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving work report");
                return StatusCode(500, new ApiResponse<object>
                {
                    Success = false,
                    Message = "Đã xảy ra lỗi khi lấy báo cáo công việc"
                });
            }
        }
        ...
        /// </summary>
        private async Task<List<TopPerformerDto>> GetTopPerformers(string userRole, int currentUserId)
        {
            try
            {
                var currentUser = await _context.Users
                ...
                // Get performance data from tasks or projects
                // This is a sample implementation - adjust based on your actual data model
                var performers = await employeeQuery
                    .Select(e => new TopPerformerDto
                    {
                        ...
                        PerformanceScore = 0 // Calculate based on completed tasks, projects, etc.
                    })
                    .Take(5)
                    .ToListAsync();

                // If no real data, return sample top performers
                if (!performers.Any() || performers.All(p => p.PerformanceScore == 0))
                {
                    return new List<TopPerformerDto>
                    {
                        new TopPerformerDto { EmployeeId = 1, EmployeeName = "Phạm Thùy Linh", DepartmentName = "Marketing", PositionName = "Trưởng phòng", PerformanceScore = 98.5m },
                        new TopPerformerDto { EmployeeId = 2, EmployeeName = "Nguyễn Văn An", DepartmentName = "IT", PositionName = "Senior Developer", PerformanceScore = 96.2m },
                        new TopPerformerDto { EmployeeId = 3, EmployeeName = "Lê Thị Mai", DepartmentName = "Design", PositionName = "Product Designer", PerformanceScore = 94.8m },
                        new TopPerformerDto { EmployeeId = 4, EmployeeName = "Trần Minh Quân", DepartmentName = "Sales", PositionName = "Account Manager", PerformanceScore = 92.4m },
                        new TopPerformerDto { EmployeeId = 5, EmployeeName = "Hoàng Bảo Trâm", DepartmentName = "HR", PositionName = "Tuyển dụng", PerformanceScore = 91.7m }
                    };
                }

                return performers.OrderByDescending(p => p.PerformanceScore).Take(5).ToList();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Lấy dữ liệu xu hướng cho dashboard
        /// </summary>
        [HttpGet("trends")]
        [Authorize]
        public async Task<ActionResult<ApiResponse<TrendsDto>>> GetTrends()
        {
            try
            {
                ...
                var currentTotalEmployees = await _context.Employees.CountAsync(e => e.Status == 1);
                var lastTotalEmployees = await _context.Employees.CountAsync(e => e.Status == 1 && e.HireDate <= endLastMonth);

                var currentNewHires = await _context.Employees
                    .CountAsync(e => e.HireDate >= startCurrentMonth);
                var lastNewHires = await _context.Employees
                    .CountAsync(e => e.HireDate >= startLastMonth && e.HireDate <= endLastMonth);

                var trends = new TrendsDto
                {
                    HrTrends = new HrTrendsDto
                    {
                        TotalEmployees = lastTotalEmployees > 0
                            ? Math.Round((currentTotalEmployees - lastTotalEmployees) / (decimal)lastTotalEmployees * 100, 1)
                            : 2.5m,
                        NewHires = lastNewHires > 0
                            ? Math.Round((currentNewHires - lastNewHires) / (decimal)lastNewHires * 100, 1)
                            : 8.3m
                    }
                };

                return Ok(new ApiResponse<TrendsDto>
                {
                    Success = true,
                    Message = "Dữ liệu xu hướng",
                    Data = trends
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving trends data");
                return StatusCode(500, new ApiResponse<object>
                {
                    Success = false,
                    Message = "Đã xảy ra lỗi khi lấy dữ liệu xu hướng"
                });
            }
        }
        ...
        public async Task<IActionResult> ExportHRReport()
        {
            try
            {
                ...
                return Ok(new { message = "Export HR report - To be implemented" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error exporting HR report");
        ...
        public async Task<IActionResult> ExportAttendanceReport()
        {
            try
            {
                // Implement export logic here
                // For now, returning a simple message
                return Ok(new { message = "Export Attendance report - To be implemented" });
            }
            catch (Exception ex)
            {
    ...
    public class WorkReportDto
    {
        ...
        public int OntimeTrend { get; set; }
        public int LateTrend { get; set; }
        public decimal AvgWorkingHours { get; set; }
        public int AvgWorkingHoursTrend { get; set; }
        public int OntimeRate { get; set; }
        public int OntimeRateTrend { get; set; }
        public int OverduePriorityCount { get; set; }
        public int OverduePriorityTrend { get; set; }
        public decimal TeamVelocity { get; set; }
        public decimal TeamVelocityTrend { get; set; }
    ...
    {
        public HrTrendsDto HrTrends { get; set; } = new();
    }

    public class HrTrendsDto
    {
        public decimal TotalEmployees { get; set; }
        public decimal NewHires { get; set; }
    }
